...
def check_in(message: types.Message):
    bot.send_message(message.from_user.id,
                     ...
                     'Пример: 01-01-2023')
    bot.set_state(message.from_user.id, Highprice.check_out_date, message.chat.id)


@bot.message_handler(state=Highprice.check_out_date)
def get_check_in_ask_check_out(message: types.Message):
    ...
    if not date_passed(date):
        ...
        bot.set_state(message.from_user.id, Highprice.city, message.chat.id)
...
@bot.message_handler(state=Highprice.city)
def get_checkout_ask_city(message: types.Message):
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        if first_date_later_that_second(date, data['check_in']):
            ...
            bot.set_state(message.from_user.id, Highprice.validate_city, message.chat.id)
        else:
            bot.send_message(message.from_user.id,
...
@bot.message_handler(state=Highprice.validate_city)
def validate_city_or_ask_amount_of_hotels(message: types.Message):
    ...
    if len(options) == 0:
        ...
        bot.set_state(message.from_user.id, Highprice.pick_city, message.chat.id)


@bot.message_handler(state=Highprice.pick_city)
...
@bot.message_handler(state=Highprice.amount_hotels)
def get_amount_of_hotels_ask_if_photos(message: types.Message):
    if message.text.isdigit() and 0 < int(message.text) <= 10:
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['amount'] = int(message.text)
            bot.set_state(message.from_user.id, Highprice.if_photos, message.chat.id)
            bot.send_message(message.from_user.id,
...
@bot.message_handler(state=Highprice.if_photos)
def if_photos(message: types.Message):
    if message.text.lower().strip() == 'да':
        ...
        bot.set_state(message.from_user.id, Highprice.how_many_photos, message.chat.id)
